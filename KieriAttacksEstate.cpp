...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
void dfs(ll curr, ll parent, vector<vector<ll>>& edges, vector<ll>& arr, map<ll,pair<ll,ll>>& dp){
     if(curr == 0){
        dp[curr] = {arr[0],arr[0]};
     }
     else{
        ll mini = min(arr[curr] - dp[parent].ff,arr[curr] - dp[parent].ss);
        ll maxi = max(arr[curr],max(arr[curr] - dp[parent].ff,arr[curr] - dp[parent].ss));
        dp[curr] = {maxi,mini};
     }
     for(auto it:edges[curr]){
        if(it != parent){
           dfs(it,curr,edges,arr,dp); 
        }
     }
}
void solve() {
